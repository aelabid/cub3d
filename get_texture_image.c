...
/*   Updated: 2023/01/10 23:31:02 by aelabid          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    get_images()
{
    texture[0].img = mlx_xpm_file_to_image(mlx.mlx_ptr, "./texture/NO.xpm", &texture[0].width, &texture[0].height);
    texture[0].addr = (int *)mlx_get_data_addr(texture[0].img, &texture[0].bits_per_pixel, &texture[0].line_length,
								&texture[0].endian);
    texture[1].img = mlx_xpm_file_to_image(mlx.mlx_ptr, "./texture/SO.xpm", &texture[1].width, &texture[1].height);
    texture[1].addr = (int *)mlx_get_data_addr(texture[1].img, &texture[1].bits_per_pixel, &texture[1].line_length,
								&texture[1].endian);
    texture[2].img = mlx_xpm_file_to_image(mlx.mlx_ptr, "./texture/EA.xpm", &texture[2].width, &texture[2].height);
    texture[2].addr = (int *)mlx_get_data_addr(texture[2].img, &texture[2].bits_per_pixel, &texture[2].line_length,
								&texture[2].endian);
    texture[3].img = mlx_xpm_file_to_image(mlx.mlx_ptr, "./texture/WE.xpm", &texture[3].width, &texture[3].height);
